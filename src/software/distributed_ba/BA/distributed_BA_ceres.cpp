...
//            MPI_Init(NULL, NULL);
            ...
            string groupDir("/home/spark/cloudleedua/output_data/groups/");
            ...
            std::cout << "Start reading group_" << world_rank << " communicator info" << std::endl;
            ...
            std::cout << "Read group_" << world_rank << " , total size: " << groups.size() << std::endl;
            ...
            std::cout << "Construct communicator map for group_" << world_rank << " , total communicators "
                      << com_map.size() << std::endl;

            int global_sum = 0;
            for (auto &com_it: com_map) {
                MPI_Allreduce(&world_rank, &global_sum, 1, MPI_INT, MPI_SUM,
                              com_it.second);
                int com_size;
                MPI_Comm_size(com_it.second, &com_size);
                std::cout << "Process of World rank " << world_rank << " , ALL REDUCE of camera " << com_it.first
                          << " . Sum of init world rank in com group: " << global_sum << " (comm size:" << com_size
                          << ")" << std::endl;
            }
