 ...
 */
    ...
    struct Distributed_ResidualErrorFunctor_Pinhole_Intrinsic_Radial_K3 {
      ...
      bool operator()(
        ...
        Mat3 R;
        ceres::AngleAxisToRotationMatrix(cam_extrinsics, R.data());
        Vec3  cam_c(cam_extrinsics[3],cam_extrinsics[4],cam_extrinsics[5]);
        Vec3  point3d(pos_3dpoint[0],pos_3dpoint[1],pos_3dpoint[2]);
        Vec3 trans_point = R * ( point3d - center);
        Eigen::Matrix<T, 3, 1> transformed_point(trans_point[0], trans_point[1], trans_point[2]);
    ...
    struct Distributed_Extrinsics_Loss{
      ...
      bool operator()(
        ...
        Eigen::Map<Eigen::Matrix<T, 6, 1>> residuals(out_residuals)
          << (extrinsics[0] - global_extrinsics[0] + latent_extrinsics[0]) * sqrt(coes->rho_r),
    ...
    struct Distributed_Intrinsics_Loss{
      ...
      bool operator()(
        ...
        Eigen::Map<Eigen::Matrix<T, 6, 1>> residuals(out_residuals)
          << (intrinsics[OFFSET_FOCAL_LENGTH] - global_intrinsics[OFFSET_FOCAL_LENGTH] + latent_intrinsics[OFFSET_FOCAL_LENGTH]) * sqrt(coes->rho_f),
